...
const { loadSourceRegistry, loadSearchIndex } = require('./cache');
const { loadConfig } = require('./config');
const { normalizeLanguage } = require('./normalize');
const { search: bm25Search } = require('./bm25');
...
function searchEntries(query, filters = {}) {
  ...
  if (_searchIndex) {
    ...
    results = deduped.map((entry) => {
      ...
      // BUG-2: Null guard for entry.name
 ...
 */
function getEntry(idOrNamespacedId, type = null) {
  const { docs, skills } = getMerged();
  let pool;
 ...
 */
function resolveDocPath(entry, language, version) {
 ...
 */
function resolveEntryFile(resolved, type) {
