...
```bash
npm run test:tier1   # unit tests (411)
npm run test:tier2   # integration + stress tests (258)
npm run test:qa      # fuzz, property, integration, chaos, mutation (96)
...
- **Property-Based Testing**: 26개 불변성 검증 — hash 결정성, 토큰 단조성, 예산 준수 등
- **Integration E2E**: 14개 전체 파이프라인 스모크 테스트 — episodic→semantic→entity→graph→cost
- **Chaos Engineering**: 15개 장애 주입 — DB 손실, 10K 레코드 압박, 동시성 경합, SQL 인젝션
- **Mutation Testing**: 17개 수동 변이체 — sanitizer, hash, token, shell 보안 로직 사멸 확인
- **Dependency Audit**: 0 vulnerabilities (path-to-regexp, picomatch, yaml 패치 완료)
