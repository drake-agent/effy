...
프로덕션 PostgreSQL을 단일 데이터베이스로 사용합니다.

- **PostgreSQL Full-Text Search**: tsvector + GIN 인덱스
- **Connection Pool**: `pg` Pool (min/max 설정, idle timeout, 백프레셔)
- **Migration**: `npm run db:migrate-pg`로 스키마 관리
...
| **데이터베이스** | PostgreSQL, tsvector Full-Text Search, Connection Pool |
...
### LLM Input Pipeline PII Scrubbing
현재 사용자 메시지가 LLM API로 전송될 때 PII(개인식별정보)가 마스킹 없이 전달됨. LLM 출력에는 secret-scrubber가 적용되지만, 입력 경로에는 미적용 상태.

**구현 필요 사항:**
- `user → LLM` 경로에 PII 필터 미들웨어 추가
- 주민번호, 카드번호, 이메일, 전화번호 등 마스킹 후 LLM에 전달
- 마스킹된 값은 응답 후 원본으로 복원하거나 `[REDACTED]` 유지
...
- **Database**: PostgreSQL, tsvector Full-Text Search
