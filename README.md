...
### 🐘 Dual-DB Adapter (SQLite + PostgreSQL)
프로덕션 환경에서 PostgreSQL을 선택할 수 있습니다. 기존 SQLite도 그대로 지원합니다.
...
- **하위 호환성**: 기존 `getDb()` 호출은 SQLite 모드에서 그대로 동작
...
환경변수로도 설정 가능: `DB_TYPE`, `DB_HOST`, `DB_PORT`, `DB_NAME`, `DB_USER`, `DB_PASSWORD`, `DATABASE_URL`

### 🔀 Composable Pipeline
에이전트 실행 흐름을 선언적으로 조합합니다.

- **PipelineBuilder**: `.addStep()`, `.addBranch()`, `.addParallel()` 체이닝
- **내장 Step**: validate, transform, classify, enrich, route, filter, aggregate
- **에러 처리**: step별 retry/fallback, 전체 circuit breaker
- **실시간 메트릭**: step별 소요 시간, 성공률 자동 수집
...
- **Communication Graph**: 에이전트 간 대화 이력 + 토폴로지 분석
- **Branch Agent**: 조건 분기 + 병렬 실행 + 결과 병합
- **Outcome Gate**: 응답 품질 미달 시 자동 재생성
- **Tool Isolation**: 에이전트별 도구 접근 권한 격리
- **Worker Inspect**: 에이전트 실행 상태 실시간 모니터링
- **Schedule Circuit Breaker**: 크론 작업 연속 실패 시 자동 차단
- **Rate Limit State**: 분산 환경 대응 속도 제한
...
- **Attachment Memory**: 첨부파일 텍스트 추출 + 메모리 저장
- **Bulletin Board**: 팀 전체 공유 메모리 보드
- **Search Modes**: 키워드, 시맨틱, 하이브리드 검색 모드

### 🌐 게이트웨이 강화

- **Health Check**: `/health` 엔드포인트, DB/LLM/메모리 상태 체크
- **Graceful Shutdown**: SIGTERM 처리, in-flight 요청 완료 후 종료
- **Request Coalescer**: 동일 요청 중복 제거 (debounce)
- **Live Logs API**: SSE 기반 실시간 로그 스트리밍

### 🔐 보안 강화 (38 + 17 패치)

v3.6.3 Review Council (38 fixes) + v3.7 DB Adapter Review Council (17 fixes):

- SQL 인젝션 방어 (FTS 화이트리스트, WHERE 절 검증, ORDER BY 세니타이징)
- ReDoS 방어 (워크플로 엔진 정규식 감사)
- Secret Scrubber (로그 내 API 키/토큰 자동 마스킹)
- SSRF Guard (도구 실행 시 내부 네트워크 접근 차단)
- Admin 권한 강화 (프로덕션 빈 adminUsers 차단)
- 에러 로그에서 SQL 텍스트 제거 (스키마 누출 방지)
...
| **데이터베이스** | SQLite + PostgreSQL Dual Adapter, FTS5/tsvector, Connection Pool |
| **관측** | OpenTelemetry (Traces + Metrics), Secret Scrubber, Health Check |
| **분산** | Message Bus, Service Discovery, Session Store, Agent Service |
...
DB_PASSWORD=secret
```
또는 `DATABASE_URL=postgres://effy:secret@localhost:5432/effy`
...
- **Database**: SQLite (기본) / PostgreSQL (선택), FTS5 / tsvector
